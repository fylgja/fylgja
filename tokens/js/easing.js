...
function replacEaseKeysToName(values) {
	const keys = ["xs", "sm", "DEFAULT", "lg", "xl"];
...
// Planned as the new default
const easingNamed = Object.fromEntries(
	Object.entries(easing).map(([name, values]) => [
		name,
		replacEaseKeysToName(values),
	]),
);

export { easing as default, easing, easingNamed };
